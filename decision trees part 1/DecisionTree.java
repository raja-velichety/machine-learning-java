...
public class DecisionTree{
    ...
    private class Node{
        ...
        //Constructor of node
        private Node(String nodeType, int value, Node left, Node right,String confidence){
    ...
    private Node build(ArrayList<Record> records, ArrayList<Integer> attributes){
        ...
        if(sameClass(records)){
           ...
           String  confidence = "Leaf: " + records.size() + "/" + numberRecords + " = " +
                            String.format("%.1f", ((records.size() / (double)numberRecords) * 100))
                            + "%" + " | Majority: " + numbermajorityClass(records) + "/" + records.size() + " = " +
                            String.format("%.1f", ((numbermajorityClass(records)/ (double)records.size()) * 100)) + "%";
            //node is leaf with that class
            node =new Node("leaf",className,null,null,confidence);
        }
        ...
        //if there are no attributes
            ...
            if(leftRecords.isEmpty() || rightRecords.isEmpty()){
                ...
                node = new Node("internal",condition,null,null,confidence);
...
private int numbermajorityClass(ArrayList<Record> records)
{
    int[] frequency = new int[numberClasses];           //frequency array
    ...
    for(int i = 0; i < numberClasses; i++)               //frequency
        ...
        return frequency[maxIndex];                                 //return number of majoriy class records
}
/*************************************************************************************/
...
/****************************************************************************************************************/
            ...
            throws IOException
    {
        ...
        for(int i=0;i<numberRecords;i++)
        {
            ...
            Node currentnode=classify(attributeArray);
            int className=currentnode.className;
            String confidence=currentnode.confidence;
            String label;
            if (className == 1)
            label = "lowrisk";
            else if (className == 2)
    ...
    public double validate(String validationFile) throws IOException
    {
        ...
        for(int i=0; i < numberRecords; i++)
        {
            ...
            Node currentnode=classify(attributeArray);
            int predictedClass = currentnode.className;
 ...
 /****************************************************************************************************************/
     ...
     public void trainingError(String trainingfile) throws IOException
    {
        ...
        for(int i = 0; i < numberRecords; i++)
        {
            ...
            Node currentnode=classify(attributeArray);
            int guessClassName = currentnode.className;
    ...
    public void leaveoneout() throws IOException{
        ...
        for(int i=0;i<total_records_length;i++){
            ...
            trainingoutFile.println((total_records_length-1)+" "+5+" "+4);
            for(int j=0;j<records.size();j++){
                if(j==i){
                    testingoutFile.println(1);
                    testingoutFile.print(records.get(j).attributes[0]+" "+records.get(j).attributes[1]+" "+records.get(j).attributes[2]+" "+records.get(j).attributes[3]+" "+records.get(j).attributes[4]+" "+records.get(j).className);
                }
                else if(j==(records.size()-1)){
                    trainingoutFile.print(records.get(j).attributes[0]+" "+records.get(j).attributes[1]+" "+records.get(j).attributes[2]+" "+records.get(j).attributes[3]+" "+records.get(j).attributes[4]+" "+records.get(j).className);
                }
                else{

                    trainingoutFile.println(records.get(j).attributes[0]+" "+records.get(j).attributes[1]+" "+records.get(j).attributes[2]+" "+records.get(j).attributes[3]+" "+records.get(j).attributes[4]+" "+records.get(j).className);
            ...
            //System.out.println(validate("leaveonetesting.txt"));
